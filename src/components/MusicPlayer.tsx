...
export function MusicPlayer({song, compact = false}:{song:Song;compact?:boolean}) {
  return <div className={`muted-surface ${compact ? 'p-3' : 'p-3.5'} rounded-xl`}>
    <div className="flex min-w-0 items-center gap-3">
      <img src={song.coverArt} alt={`${song.title} cover art`} className={`${compact ? 'h-12 w-12' : 'h-14 w-14 sm:h-16 sm:w-16'} shrink-0 rounded-lg object-cover`} />
      <button className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-neon text-black transition hover:scale-105" aria-label={`Play ${song.title}`}><Play size={18} fill="currentColor" /></button>
